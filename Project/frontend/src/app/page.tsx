...
export default function Home() {
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        /* COMMENTED: Send audio to backend API
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-gray-50 via-purple-50 to-gray-100">
      {/* Navigation Bar */}
        ...
        <div className="h-full max-w-7xl mx-auto">
          <div className="h-full bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
            <div className="h-full grid lg:grid-cols-4">
              ...
              <div className="lg:col-span-3 h-full flex flex-col min-h-0">
                    ...
                    {messages.map((message, index) => (
                      message.sender === 'bot' ? (
                        <div key={message.id} className="flex items-start space-x-3 opacity-0 animate-[fadeSlideIn_0.6s_ease-out_forwards]" style={{ animationDelay: `${index * 0.02}s` }}>
                          <div className="w-8 h-8 bg-gradient-to-br from-purple-300 to-purple-400 rounded-full flex-shrink-0"></div>
                        ...
                        <div key={message.id} className="flex items-start space-x-3 justify-end opacity-0 animate-[fadeSlideIn_0.6s_ease-out_forwards]" style={{ animationDelay: `${index * 0.02}s` }}>
                          <div className="flex flex-col space-y-1 max-w-lg items-end">
                            <div className="bg-gradient-to-br from-purple-300 to-purple-400 rounded-2xl rounded-tr-sm px-4 py-3 shadow-sm">
                              {message.text ? (
                ...
                <div className="flex-shrink-0 px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                  <div className="flex items-center space-x-3 bg-white rounded-2xl border border-gray-200 px-4 py-3 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <button 
                      onClick={handleSend}
                      className="px-4 py-2 bg-gradient-to-r from-purple-300 to-purple-400 text-white rounded-xl font-medium hover:from-purple-400 hover:to-purple-500 transition-all hover:shadow-lg active:scale-95"
                      aria-label="send"
                    >
              ...
              {/* Microphone Sidebar */}
              <div className="hidden lg:flex bg-gradient-to-b from-purple-50/50 to-gray-50/30 border-l border-gray-100 p-6 items-center justify-center">
                <button 
      ...
      <style jsx global>{`
        ...
        .audio-player-wrapper .rhap_controls-section {
          display: flex;
          align-items: center;
          gap: 6px; /* 👈 closes the gap between button and bar */
        }
